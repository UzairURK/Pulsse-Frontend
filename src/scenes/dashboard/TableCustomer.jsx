...
function TableCustomer() {
  ...
  const rows_ = [
    // this is dummy data, whenever we get real data change "rows_" to "rows"
    {
  ...
  const filteredRows = rows.filter(row => {
    const searchData =
      `${row.visitDate} ${row.visits} ${row.gender} ${row.age} ${row.group} ${row.timeIn} ${row.timeOut} ${row.stay} ${row.action}`.toLowerCase()
  ...
  const formatTime = (timeStr) => {
    // Regular expression to match the time format "HH:mm:ss.SSSSSS"
  const timeRegex = /^([01]\d|2[0-3]):([0-5]\d):([0-5]\d)\.\d{6}$/;

  // Check if timeStr is in the expected format
  if (!timeRegex.test(timeStr)) {
    // Return empty string for invalid time format
    return 'None';
  }
    // Assuming timeStr is in the format "17:46:30.362487"
    const timestamp = new Date(`2000-01-01T${timeStr}Z`);
              ...
              >
                ...
                <Box className='flex'>
                  ...
                  >
                    ...
                    >
                      <path
                        d='M18.493 9.10596L11.9395 15.5971L18.493 22.0883'
                        stroke='#AEAEAE'
                        stroke-width='1.5'
                ...
                {currentPage > 1 && (
                  <span onClick={() => setCurrentPage(currentPage - 1)}>&laquo;</span>
                )}
                ...
                {currentPage < totalPages && (
                  <span onClick={() => setCurrentPage(currentPage + 1)}>&raquo;</span>
                )}
              </div>
                  <div
                    className='ml-2 flex mt-2'
                ...
                >
                  ...
                  <TableBody>
                    {rows_.map((row, index) => (
                      ...
                      <TableCell sx={rowCellStyle}>{getGroupStatus(row.group)}</TableCell>
